...
def rasterize_sphere(center, radius, fill=False):
    ...
    # Iterate x within [-r, r] (Bounding box width)
    for dx in range(-r, r + 1):
        dx2 = dx * dx
        # Calculate remaining squared radius for y and z: r^2 - x^2
        rem_y = r_sq - dx2
        if rem_y < 0:
            continue

        # Exact bound for y: |y| <= sqrt(r^2 - x^2)
        dy_limit = math.isqrt(rem_y)
        for dy in range(-dy_limit, dy_limit + 1):
            dy2 = dy * dy
            # Calculate remaining squared radius for z: r^2 - x^2 - y^2
            rem_z = rem_y - dy2
            if rem_z < 0:
                continue

            # Exact bound for z: |z| <= sqrt(r^2 - x^2 - y^2)
            dz_limit = math.isqrt(rem_z)
            x = cx + dx
            y = cy + dy

            if fill:
                # Iterate all z in the vertical column within the sphere
                for dz in range(-dz_limit, dz_limit + 1):
                    points.append((x, y, cz + dz))
            else:
                # Hollow sphere logic:
                # We want points where (r-1)^2 <= x^2 + y^2 + z^2 <= r^2
                # This implies z^2 >= (r-1)^2 - x^2 - y^2
                rem_inner = r_inner_sq - dx2 - dy2
                if rem_inner <= 0:
                    # The inner void doesn't reach this column; the whole column is part of the shell.
                    for dz in range(-dz_limit, dz_limit + 1):
                        points.append((x, y, cz + dz))
                else:
                    # There is a void in the middle of this column.
                    # We need |z| >= sqrt(rem_inner)
                    dz_inner_limit = math.isqrt(rem_inner)
                    if dz_inner_limit * dz_inner_limit < rem_inner:
                        dz_inner_limit += 1

                    # Add bottom segment of the shell
                    for dz in range(-dz_limit, -dz_inner_limit + 1):
                        points.append((x, y, cz + dz))
                    # Add top segment of the shell
                    for dz in range(dz_inner_limit, dz_limit + 1):
                        points.append((x, y, cz + dz))

...
def rasterize_cylinder(base, radius, height, axis="y", fill=False):
    ...
    # Scan the 2D box for the cross-section
    for u in range(-r, r + 1):
        for v in range(-r, r + 1):
            dist_sq = u * u + v * v
            if dist_sq <= r_sq:
                disk_offsets.append((u, v))
                if not fill and dist_sq >= inner_r_sq:
                    ring_offsets.append((u, v))

    if fill:
        # If filled, we just extrude the disk
        extrusion_offsets = disk_offsets
    else:
        # If hollow, we need the ring for the body
        extrusion_offsets = ring_offsets

    # Generate points based on axis
    if axis == "y":
        for y_offset in range(h):
            y = by + y_offset
            # Caps are always solid disks if h > 0
            is_cap = (y_offset == 0) or (y_offset == h - 1)
            offsets = disk_offsets if is_cap else extrusion_offsets

            for u, v in offsets:
                # axis y -> u=x-bx, v=z-bz
                points.append((bx + u, y, bz + v))

    elif axis == "x":
        for x_offset in range(h):
            x = bx + x_offset
            is_cap = (x_offset == 0) or (x_offset == h - 1)
            offsets = disk_offsets if is_cap else extrusion_offsets

            for u, v in offsets:
                # axis x -> u=y-by, v=z-bz
                points.append((x, by + u, bz + v))

    elif axis == "z":
        for z_offset in range(h):
            z = bz + z_offset
            is_cap = (z_offset == 0) or (z_offset == h - 1)
            offsets = disk_offsets if is_cap else extrusion_offsets

            for u, v in offsets:
                # axis z -> u=x-bx, v=y-by

...
## 2026-01-07 - Line Drawing Deduplication
**Learning:** Naively sweeping a shape (like a sphere for line thickness) by placing it at every interval creates massive object redundancy (~3.6x overhead for thickness=10). Because the `Module` structure is a simple list, it doesn't automatically deduplicate.
**Action:** For any sweeping geometry generation, pre-calculate the brush shape offsets and use a `set` to collect unique absolute positions *before* instantiating blocks. This reduces both memory usage and processing time significantly.
